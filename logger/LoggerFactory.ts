...
class DevLoggerManager implements LoggerManager {
  ...
  create(name: string, opts?: LoggerOptions): Logger {
    ...
    if (this.enablePackagePrefix) {
      const { logger: packageLogger, name: packageLoggerName } =
        this.getOrCreatePackageLogger();
    ...
    const level =
      opts?.level ||
      this.levelMappings.find((e) => e.name === name)?.level ||
      this.defaultLevel;
...
class LiveLoggerManager implements LoggerManager {
  ...
  setLevel(): void {
    this.rootLogger.warn(
      "Unexpected LoggerFactory setting - setLevel is not supported for LIVE loggers",
    );
  }

  addLevelMapping(): void {
    this.rootLogger.warn(
      "Unexpected LoggerFactory setting - level mapping is not supported for LIVE loggers",
    );
  }

  createRoot(): PinoLogger {
    return new PinoLogger(
      pino({ level: this.defaultLevel }, pinoLambdaDestination()),
    );
  }
