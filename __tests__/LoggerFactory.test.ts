...
describe('LoggerFactory', () => {
    ...
    it('should consider add-mapping', async () => {
        ...
        // pretty format has ansi colour codes and formatted segments
        const regex = /\[[0-9:.]{12}] \x1b\[32mINFO\x1b\[39m \(logging\/test-mapping1\/[0-9]+\): \x1b\[36mlogger1\x1b\[39m/;
    ...
    it('should support set-level', async () => {
        ...
        // pretty format has ansi colour codes and formatted segments
        expect(lines[0]).toMatch(/\[[0-9:.]{12}] \x1b\[32mINFO\x1b\[39m \(logging\/test-mapping1\/[0-9]+\): \x1b\[36mfirst-logger1\x1b\[39m/);
        expect(lines[1]).toMatch(/\[[0-9:.]{12}] \x1b\[32mINFO\x1b\[39m \(logging\/test-mapping2\/[0-9]+\): \x1b\[36msecond-logger2\x1b\[39m/);
    })

    it('should support set-level on parent', async () => {
        // the LOG_DESTINATION env-var allows us to write to a file
        const outputFile = path.join(tempDir, 'test-logs-with-set-level-parent.log');
        process.env.LOG_DESTINATION = outputFile;
        process.env.NODE_ENV = 'development';
        ...
        // pretty format has ansi colour codes and formatted segments
        const regex = /\[[0-9:.]{12}] \x1b\[32mINFO\x1b\[39m \(logging\/test-child\/[0-9]+\): \x1b\[36mlogger-child-is-here\x1b\[39m/;
